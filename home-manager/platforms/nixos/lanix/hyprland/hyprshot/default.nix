...
}:
{
  ...
  wayland.windowManager.hyprland = {
    settings = {
      bind = [
        "$mainMod CTRL, 4, exec, ${lib.getExe pkgs.hyprshot} --mode region --raw | ${lib.getExe pkgs.satty} --filename -"
        "$mainMod CTRL, 3, exec, ${lib.getExe pkgs.hyprshot} --mode window --raw | ${lib.getExe pkgs.satty} --filename -"
      ];
    };
  };
}
